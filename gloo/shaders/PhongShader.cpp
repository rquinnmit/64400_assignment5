...
void PhongShader::SetTargetNode(const SceneNode& node,
  ...
  // Default shadow mapping to disabled (will be enabled by SetShadowMapping if needed)
  SetUniform("shadow_mapping_enabled", false);
...
void PhongShader::SetShadowMapping(
  ...
  // Bind shadow texture to texture unit 3 (0, 1, 2 are used for material textures)
  shadow_texture.BindToUnit(3);
  SetUniform("shadow_map", 3);
  SetUniform("world_to_light_ndc_matrix", world_to_light_ndc_matrix);
  SetUniform("shadow_mapping_enabled", true);
}
}  // namespace GLOO
